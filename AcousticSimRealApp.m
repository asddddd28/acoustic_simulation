classdef AcousticSimRealApp < handle
    % ACOUSTICSIMREALAPP 声场生成和波束成形实时仿真系统 (MATLAB版) - 修复版
    ...
    methods
        function obj = AcousticSimRealApp()
            % 构造函数：初始化界面和数据
            obj.initData();
            obj.createUI();
            obj.initTimer();
        ...
        function createUI(obj)
            % 创建 UI 界面
            obj.UIFigure = uifigure('Name', '声场生成与波束成形仿真系统 (MATLAB修复版)', ...
            ...
            % 状态栏
            obj.StatusLabel = uilabel(vbox, 'Text', '就绪 | 提示: 实时模式下左侧图可交互，WASD移动声源');
        ...
        function onRealTimeToggle(obj, src, ~)
            if strcmp(src.Value, 'On')
                ...
                % 4. 启动定时器
                start(obj.SimTimer);
                obj.StatusLabel.Text = '实时仿真运行中... (使用 WASD 移动声源)';
        ...
        function [final_map_dB, max_val, GridX, GridY] = calculateBeamformingMap(obj, snr)
            ...
            if strcmp(algo, 'MUSIC') || strcmp(algo, 'Functional')
                ...
                elseif strcmp(algo, 'Functional')
                    ...
                    % Functional Beamforming 对特征值敏感，减去底噪可显著提升对比度
            ...
            % 缓存网格优化
            if isempty(obj.CachedGridX) || size(obj.CachedGridX, 1) ~= length(-0.2:grid_res:0.2)
        ...
        function bf_map = computeLayer(obj, M_core, GX, GY, GZ, k, model, algo, gamma)
            ...
            % 向量化计算优化可能性：此处保留循环逻辑以确保内存稳定，但已简化
            for i = 1:n_points
                focus_pt = grid_pts(:, i);
                dx = focus_pt(1) - mic_x;
                dy = focus_pt(2) - mic_y;
